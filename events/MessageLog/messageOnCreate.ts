...
async function afterSendErrorHandler(
	message: Message,
	client: Client,
	type: string,
	values: any,
	...
	if (type === 'DM') {
		message.react('✅');
		processing?.remove();
		const embed = await errorEmbedAsemblyClient(
			message,
			client,
			values,
			locales
...
async function sendToDMChannel(
	...
	for (const id of channels) {
		try {
			...
			sendDBWrite(ticketNumberDatabse, message, msh);
		} catch (e) {
			errorSender.push(id);
		}
	}
...
async function sendToServer(
	...
	const ticketNumberDatabse = await lib.ticket.get(message.channelId);
	const reciverData = await lib.db
		.table(`tt_${ticketNumberDatabse}`)
		.get('info');
...
async function infoWriter(
	...
	if (x === 'toServer') {
		...
		await lib.db
			.table(`tt_${ticketNumberDatabse}`)
			.add('messageAnalitys.messages.sentByDM', 1);
		await lib.db
			.table(`tt_${ticketNumberDatabse}`)
		...
		await lib.db
			.table(`tt_${ticketNumberDatabse}`)
			.set('activity.lastDMMessage', lib.unixTimestamp());
		return;
	}
	if (x === 'toDM') {
		await lib.db.table(`tt_${ticketNumberDatabse}`).set(message.id, {
		...
		await lib.db
			.table(`tt_${ticketNumberDatabse}`)
			.add('messageAnalitys.messages.sentByServer', 1);
		await lib.db
			.table(`tt_${ticketNumberDatabse}`)
		...
		await lib.db
			.table(`tt_${ticketNumberDatabse}`)
			.set('activity.lastServerMessage', lib.unixTimestamp());
	}
}

async function sendDBWrite(
	ticketNumberDatabse: number,
	message: Message,
	msh: Message
) {
