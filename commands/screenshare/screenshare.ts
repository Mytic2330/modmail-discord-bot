...
module.exports = {
	...
	async execute(interaction: CommandInteraction) {
		...
		if (!member?.voice.channel) {
			interaction.reply({
				content: 'Uporabnik ni povezan v kanal!',
				ephemeral: true,
			});
			return;
		}
		...
		for (const id of member.roles.cache) {
			if (id[0] === role) {
				interaction.reply({
					content: 'Uporabnik že ima screenshare rolo!',
					ephemeral: true,
				});
				return;
			}
		...
		for (const id of usersWithRole) {
			if (id === member.id) {
				interaction.reply({
					content: 'Uporabnik je že dobil screenshare rolo!',
					ephemeral: true,
				});
				return;
			}
		...
		try {
			...
			console.error(e);
			interaction.reply({
				content: 'Napaka pri dodajanju role!',
				ephemeral: true,
			});
		}
